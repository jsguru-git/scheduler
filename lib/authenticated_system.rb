...
protected
        ...
        respond_to do |format|
            format.html do
                ...
                if Rails.env.production?
                    redirect_to new_session_url(:protocol => 'https://')
                else
    ...
    def store_location
        session[:return_to] = request.url if params[:format] != 'json' && params[:format] != 'js' && params[:format] != 'xml'
    end
    ...
    def permission_granted
        logger.info("[authentication] Permission granted to %s at %s for %s" %
          [(logged_in? ? current_user.id : 'guest'), Time.now, request.url])
    end
